                            ...
                            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                                <div class="md:col-span-1">
                                    <label for="full_name">Rol.(*)<label>
                                            <input type="text" id="rol" required name="name" value="{{$rol->name}}"
                                ...
                                <div class="md:col-span-6">
                                    ...
                                    <textarea type="text" name="description" rows="4"
                                        class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">{{$rol->description}}</textarea>
                                </div>
...
<!-- permisos -->
                ...
                <div class="flex max-h-[200px] w-full flex-col overflow-y-scroll">
                 @foreach ($permissions as $permission)    
                    <div class="mt-4 flex items-center gap-3">
                        ...
                        checked:before:translate-x-[22px] hover:cursor-pointer checked:bg-brand-500 dark:checked:bg-brand-400' name="permissions[]" value="{{ $permission->id }}" {{ $rol->hasPermissionTo($permission) ? 'checked' : '' }}>
        ...
        <div class="w-1/2 inline-block mt-5 w-[49.8%] rounded-xl border border-gray-200 bg-white py-4 px-2 shadow-md shadow-gray-100">
            <div class="flex items-center justify-between px-2 text-base font-medium text-gray-700">
                <div>Permisos Produccion</div>
             
            </div>
            <div class="mt-4">
                <div class="flex max-h-[200px] w-full flex-col overflow-y-scroll">
                 @foreach ($permissionsProd as $permission)    
                    <div class="mt-4 flex items-center gap-3">
                        <label>
                        <input type="checkbox" class='relative h-5 w-10 appearance-none rounded-[20px] bg-[#e0e5f2] outline-none transition duration-[0.5s] 
                        before:absolute before:top-[50%] before:h-4 before:w-4 before:translate-x-[2px] before:translate-y-[-50%] before:rounded-[20px]
